...
export default (quests) => {
  ...
  const quest = {
    ...
    uu: quests?.QuestRiftValour?.is_eclipse_mode || false,
  };
  ...
  return `<div>${quest.uu ? 'Ultimate Umbra' : ''}</div> ${text} <div class="stats">Speed ${quest.speed} · Sync ${quest.sync} · Siphon ${quest.siphon}</div>`;
};
